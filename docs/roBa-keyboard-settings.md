...
**アクセス方法**:
- **自動**: トラックボール移動時に自動的に有効化（1000ms後に自動復帰）
- **手動**: `L`キー長押しで一時移行
...
- **自動マウス機能**: `CONFIG_ZMK_MOUSE_AUTO_LAYER=y` で有効化
- **タイムアウト**: 1000ms（`automouse-timeout-ms`）
- **ターゲットレイヤー**: レイヤー4（`CONFIG_ZMK_MOUSE_AUTO_LAYER_TARGET=4`）
- **移動検知閾値**: 1（`CONFIG_PMW3610_MOVEMENT_THRESHOLD=1`）
